...
(function attachBackgroundWorker(global) {
  ...
  var backendBaseUrl = extension.backendBaseUrl || 'http://localhost:8000';
  var contextMenuId = 'eatwhat-save-to-cooldown';

  async function postJson(path, payload) {
    var response = await fetch(backendBaseUrl + path, {
      method: 'POST',
      headers: {
  ...
  chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (!message || message.type !== 'ingest_video') {
      return;
    }
    postJson('/ingest', message.payload)
      .then(async function (data) {
        if (logger) {
          await logger.log('background', 'INFO', 'ingest 上报成功', {
      ...
      .catch(async function (error) {
        if (logger) {
          await logger.log('background', 'ERROR', 'ingest 上报失败', {
            trigger: message.trigger || 'unknown',
            tab_id: sender.tab && sender.tab.id,
            url: sender.tab && sender.tab.url,
            error: error.message
