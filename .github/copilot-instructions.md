...
**EatWhat** 是一个基于 **OpenClaw 智能体框架** 的认知健康应用，让用户审视自己在 小红书的浏览足迹、看见信息茧房之外的视角、夺回深度思考的主动权。三个 MVP 功能：认知体检报告、平行书架、冷静期盒子。
...
2. **可解释**：所有 AI 结论都要带 evidence（笔记引用），不要做黑盒
3. **本地优先**：扩展只抓公开页面 DOM，不调 小红书 API、不抓登录态
...
- 写**为什么**而不是**做什么**（例：`// 小红书 SPA 路由切换后 DOM 重渲染，要 retry 选择器`）
